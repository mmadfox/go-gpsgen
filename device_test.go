...
func TestDevice_AddSensor(t *testing.T) {
	...
	tests := []struct {
		name    string
		args    args
		assert  func(d *Device, sensorID string)
		wantErr bool
	}{
		{
		...
		},
		{
			...
			assert: func(d *Device, sensorID string) {
				...
				s1, ok := d.SensorByID(sensorID)
				require.True(t, ok)
				require.Equal(t, sensorID, s1.ID())
				state := d.State()
				require.Len(t, state.Sensors, 1)
				require.Equal(t, "s1", state.Sensors[0].Name)
				require.NotEmpty(t, state.Sensors[0].Id)
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			d, err := NewDevice(nil)
			require.NoError(t, err)
			got, err := d.AddSensor(tt.args.name, tt.args.min, tt.args.max, tt.args.amplitude, 0)
			if (err != nil) != tt.wantErr {
				t.Errorf("Device.AddSensor() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if tt.wantErr {
				return
...
func TestDevice_RemoveSensor(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			arrange: func(d *Device) string {
				sid, err := d.AddSensor("s1", 1, 2, 4, 0)
				require.NoError(t, err)
				return sid
			},
			assert: func(d *Device, sid string) {
...
func TestDevice_GetSensor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			arrange: func(d *Device) string {
				sid, err := d.AddSensor("s1", 1, 2, 8, 0)
				require.NoError(t, err)
				return sid
			},
			assert: func(d *Device, sid string) {
...
func TestDevice_Update(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			arrange: func(d *Device) {
				d.AddSensor("s1", 1, 2, 8, 0)
				d.AddSensor("s2", 0.1, 1, 16, 0)
...
func TestDevice_Snapshot(t *testing.T) {
	...
	d, err := NewDevice(opts)
	require.NoError(t, err)
	d.AddSensor("s1", 1, 10, 8, 0)
	d.AddSensor("s2", 15, 20, 4, 0)
	d.AddRoute(testRoutes()...)
	for i := 0; i < 10; i++ {
